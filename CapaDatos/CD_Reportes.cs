...
namespace CapaDatos
{
    public class CD_Reportes
    {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader()) {
                        while (dr.Read()) {
                            lista.Add(new ReporteProducto()
                            {
                                ...
                                PrecioCatalogo = Convert.ToDecimal(dr["Precio Catalogo"].ToString(), new CultureInfo("es-PE")).ToString("N", formato)
                            });
                        }
        ...
        public List<ReporteVenta> ReporteVenta(string FechaInicio, string FechaFin, int IdTienda)
        {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            lista.Add(new ReporteVenta()
                            {
                                ...
                                DocumentoReferencia = dr["Referencia"].ToString()
                            });
                        }
        ...
        public List<ReportePedido> ReportePedido(string FechaInicio, string FechaFin, int IdTienda)
        {
            List<ReportePedido> lista = new List<ReportePedido>();
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                SqlCommand cmd = new SqlCommand("usp_rptPedido", oConexion);
                cmd.Parameters.AddWithValue("@FechaInicio", FechaInicio);
                cmd.Parameters.AddWithValue("@FechaFin", FechaFin);
                cmd.Parameters.AddWithValue("@IdTienda", IdTienda);
                cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            lista.Add(new ReportePedido()
                            {
                                ...
                                CantidadUnidadesVendidas = dr["Cantidad Unidades Vendidas"].ToString(),
                                CantidadProductos = dr["Cantidad Productos"].ToString(),
                                Total = Convert.ToDecimal(dr["Total Venta"].ToString(), new CultureInfo("es-PE")).ToString("N", formato),
                                DocumentoReferencia = dr["Referencia"].ToString()
                            });
                        }
